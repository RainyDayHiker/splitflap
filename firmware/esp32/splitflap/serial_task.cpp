...
SerialTask::SerialTask(SplitflapTask &splitflap_task, const uint8_t task_core) : Task("Serial", 16000, 1, task_core),
																				 Logger(),
...
void SerialTask::run()
{
	...
	while (1)
	{
		...
		while (xQueueReceive(log_queue_, &log_string, 0) == pdTRUE)
		{
			...
			if (logInterface_)
			{
				logInterface_->onLog(*log_string);
			}
			delete log_string;
		}
...
void SerialTask::setLogInterface(Log *logInterface)
{
	logInterface_ = logInterface;
}
