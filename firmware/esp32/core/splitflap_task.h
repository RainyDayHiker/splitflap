...
struct SplitflapModuleState
{
	State state;
	uint8_t flap_index;
	uint16_t offset; // module offset in motor steps
	bool moving;
	bool home_state;
	...
	{
		return state == other.state && flap_index == other.flap_index && offset == other.offset && moving == other.moving && home_state == other.home_state && count_unexpected_home == other.count_unexpected_home && count_missed_home == other.count_missed_home;
	}
