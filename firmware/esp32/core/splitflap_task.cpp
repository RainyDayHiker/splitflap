...
void SplitflapTask::runUpdate()
{
	...
	else
	{
		...
		{
			modules[i]->Update();
			bool is_idle = modules[i]->state == PANIC || modules[i]->state == STATE_DISABLED || modules[i]->state == LOOK_FOR_HOME || modules[i]->state == SENSOR_ERROR || (modules[i]->state == NORMAL && modules[i]->current_accel_step == 0);

			bool is_stopped = modules[i]->state == PANIC || modules[i]->state == STATE_DISABLED || modules[i]->current_accel_step == 0;
...
void SplitflapTask::updateStateCache()
{
	...
	for (uint8_t i = 0; i < NUM_MODULES; i++)
	{
		...
		new_state.modules[i].offset = modules[i]->GetOffset();
